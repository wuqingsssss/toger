...
<span class="waiting"><img class="icon-spin animate-spin" src="catalogm/view/theme/<?php echo $this->config->get("config_template"); ?>/images/waiting.png"/></span>
    <div id="content" class="pg-page checkout-page" pg-name="checkout">
             ...
             <span data-role="fieldcontain" class="result demo demo-select-opt" style="line-height: 0.5rem;">
               ...
               <?php foreach($dates as $dkey=> $date){?>	 
                ...
                <?php foreach($date[times] as $ttkey=> $tt){?>	 
                    <option value="<?php echo $ttkey;?>" <?php if($select_date==$ttkey&& false) echo ' selected="selected"';?>><?php echo $tt;?></option>
                 <?php }?>
                </optgroup>
                 <?php }?>	
            </select>	
     ...
     <?php if($coupons) {?>
    <div class="pg-page coupon-select">
        <?php echo $coupons?>
    </div>
    <?php }?>
...
$(function(){
    ...
    $('#uc_body .uc-coupon .coupon-select').bind('click', function() {
        var $this = $(this);
    	...
    	$.ajax({
    		...
    		success: function(json) {
    			if (json['error']) {
        			...
        			if( json['output']){
            			$('.pg-page.payment').html(json['output']);
        			}
    				checkoutComfirm();			
    				pages.switchTo(0);
    			}
	...
	$('.order_distribution .o_d_information').bind('click', function() {
		window.location = "index.php?route=checkout/checkout/changeShippingMethod";
	});
    ...
    $('#submit-pay').live('click', function () {
    	...
    	$('#submit-pay').attr({"disabled":"true"});
     	$.ajax({
    		url: 'index.php?route=checkout/checkout/paysubmit&token=<?php echo $token; ?>',
    		type: 'post',
    		data: '',
    		dataType: 'json',
    		beforeSend: function() {
	...
	console.log('checkoutComfirm',json);
				if (json['output']) {
