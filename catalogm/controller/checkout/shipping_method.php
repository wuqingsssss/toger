...
class ControllerCheckoutShippingMethod extends Controller {
  	public function index() {
		$this->load_language('checkout/shipping_method');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'GET') {
		    if( isset($this->request->get['address_id'])){
		        ...
		        $this->cart->setAdditionalDate("");//切换地址清空配送时间
		        $this->goback();
...
//					}
		...
		$this->data['action'] = $this->url->link('checkout/shipping_method', '');
  	...
  	public function insert() {
        ...
        $this->load->model('account/address');
  	    ...
  	    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
  	    	$this->log_sys->info(serialize($this->request->post));
  	    	if ((strlen(utf8_decode($this->request->post['firstname'])) < 1) || (strlen(utf8_decode($this->request->post['firstname'])) > 32)) {
  	    	...
  	    	}elseif(!$this->request->isPhone($this->request->post['mobile'])){
  	    		$json['error']['warning'] = $this->language->get('error_mobile_length');
  	    	}
