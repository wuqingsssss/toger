...
class ControllerPaymentPPStandard extends Controller {
	protected function index() {
		...
		if ($order_info) {
			...
			$total = $this->currency->format($order_info['total'] - $sub_total['total'], $currency, false, false);
	...
	protected function reorder() {
		...
		if ($order_info) {
			...
			$this->data['lc'] = $this->session->data['language'];
			$this->data['return'] = $this->url->link('account/paysuccess');
			$this->data['notify_url'] = $this->url->link('payment/pp_standard/callback');
			$this->data['cancel_return'] = $this->url->link('account/order/info', '&order_id='.$order_id, 'SSL');
	...
	public function callback() {
		...
		if ($order_info) {
			...
			if (!$response) {
				$this->log_payment->info('PP_STANDARD :: CURL failed ' . curl_error($curl) . '(' . curl_errno($curl) . ')');
			}
					
			if ($this->config->get('pp_standard_debug')) {
				$this->log_payment->info('PP_STANDARD :: IPN REQUEST: ' . $request.'PP_STANDARD :: IPN RESPONSE: ' . $response);
			}
