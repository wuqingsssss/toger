...
class ControllerPaymentAlipay extends Controller {
	...
	public function wapcallback() {
		...
		if($verify_result) {
			...
			//urldecode
			$notify_data=str_replace('&gt;','>',str_replace('&lt;','<',$_POST['notify_data']));
...
/*			else if($trade_status == 'WAIT_SELLER_SEND_GOODS') {
	...
	public function getPaymentURL($setting) {
		...
		$this->log_payment->info('getPaymentURL'.serialize($setting));
		
        if($this->detect->is_andorid_app('20002')){
        	
        	//卖家支付宝帐户
        	$seller_email = $this->config->get('alipay_seller_email');
        	$notify_url     =HTTP_SERVER . 'apiv3/notify/alipay/alipayappnotify.php';
...
navigator.alipay.pay(data,function(msgCode){
		  		navigator.splashscreen.hide();
		  		if(msgCode=="9000"){
		  		_.toast("支付成功");window.location="index.php?route=checkout/success&order_no='.$order_id.'";}
		  				else
		  				{
                      window.location="index.php?route=checkout/reorder&order_id='.$order_id.'"
        }
        },function(msg){
		  		navigator.splashscreen.hide();   
	    ...
	    $notify_url     =HTTP_SERVER . 'apiv3/notify/alipay/alipaywapnotify.php';
	    $call_back_url	=HTTP_SERVER . 'index.php?route=checkout/success';
	...
	//	$this->data['action'] = $html_text;
		...
		$this->log_payment->info('getPaymentURL:$html_text:'.$html_text);
