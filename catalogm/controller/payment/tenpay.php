...
class ControllerPaymentTenpay extends Controller {
	...
	private function pay($data=array()) {
		$this->log_payment->debug("Tenpay :: exciting tenpay init.");
	...
	public function callback() {
		...
		$this->log_payment->debug("Tenpay :: exciting callback function.");
		$cmdno = $this->config->get('tenpay_cmdno');  
		if($cmdno==1){
			$this->log_payment->debug("Tenpay :: exciting callback function pay.");
	...
	private function func_pay($order_status){
			...
			$this->log_payment->debug("Tenpay :: exciting PayResponseHandler.");
		//判断签名
			if($resHandler->isTenpaySign()) {
					$this->load->model('checkout/order');
					// 获取订单号
					$order_id = $resHandler->getParameter("sp_billno");
					$this->log_payment->debug(' order_id '.$order_id);
					$this->load->model('checkout/order');
					$order_info = $this->model_checkout_order->getOrder($order_id);
				if ($order_info) {
					...
					if( "0" == $pay_result ) {
						$this->log_payment->debug(' pay_result '.$pay_result);
						//------------------------------
						//处理业务开始
						//------------------------------
						$this->log_payment->debug(' order_status_id '.$this->config->get('tenpay_order_status_id').' order_status_id '.$order_status_id);
