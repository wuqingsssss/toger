...
class ControllerPaymentMoneybookers extends Controller {
	protected function index() {
		...
		$this->data['action'] = 'https://www.moneybookers.com/app/payment.pl?p=Shopilex';
	...
	public function callback() {
		...
		if ($order_info) {
			...
			if ($md5hash == $md5sig) {
				...
				$this->log_payment->info("md5sig returned ($md5sig) does not match generated ($md5hash). Verify Manually. Current order state: " . $this->config->get('config_order_status_id'));
			}
		}
	}
}
