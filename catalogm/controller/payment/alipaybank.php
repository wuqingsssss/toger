...
class ControllerPaymentAlipayBank extends Controller {
	public function index() {
		...
		if(isset($this->session->data['pay_bank'])){
			$parameter = array(
				...
				"subject"        => $item_name.'订单编号 - ' . $order_id ,
				"body"           => $item_name,
				"out_trade_no"   => $order_id,
				"price"          => $amount,
				"payment_type"   => "1",
				"paymethod"      =>'bankPay',
				"defaultbank"    => $this->session->data['pay_bank'],
				"quantity"       => "1",
				"logistics_fee"      =>'0.00',
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/alipaybank.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/alipaybank.tpl';
		} else {
			$this->template = 'default/template/payment/alipaybank.tpl';
		}
