...
//                'total' => $this->currency->format($result['total'], $result['currency_code'], $result['currency_value']),
    ...
    public function info() {
        ...
        if ($order_info) {
           ...
           $this->data['order_total']=$this->currency->format( $order_info['total'], $order_info['currency_code'], $order_info['currency_value']);
            
            $refund_order_array = array($this->config->get('config_order_status_id'));
            
            if (in_array( $order_info['order_status_id'], $refund_order_array)) {//&&(stripos($order_info['payment_code'],'alipay')===FALSE)
            	$refund = true;
            } else {
            	$refund = false;
            }
            
            $this->data['refundable'] = $refund;
            $this->data['refund'] = $this->url->link('account/order/refund', '', 'SSL');
            
            $this->data['status_id'] = $order_info['order_status_id'];
            $this->data['status'] = $order_info['status'];
            ...
            {  	
                $this->data['payment'] = $this->getChild('payment/' . $order_info['payment_code'] . '/reorder',array('order_id'=>$order_id));
            }
            ...
            */
            ...
            foreach ($products as $product) {
                ...
                $this->data['products'][] = array(
                	...
                	'promotion_price' =>(int)$product['promotion_price']? $this->currency->format($product['promotion_price'], $order_info['currency_code'], $order_info['currency_value']):0,
            ...
            $this->data['reorder_action']   = $this->url->link('checkout/reorder','','SSL');
