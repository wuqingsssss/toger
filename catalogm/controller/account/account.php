...
class ControllerAccountAccount extends Controller { 
	public function index() {
		...
		if (!$this->customer->isLogged()) {
	  		//$this-> $this->url->link('account/account', '', 'SSL');
...
//				'name'       => $result['firstname'] . ' ' . $result['lastname'],
		...
		$coupons = $this->model_account_coupon->getCouponsByCustomerAll($this->customer->getId());
		$this->data['c_count'] = count($coupons);
	...
	public function setup() {
		if (!$this->customer->isLogged()) {
			//$this->session->data['redirect'] = $this->url->link('account/account/setup', '', 'SSL');
			$this->setback(true, $this->url->link('account/account/setup', '', 'SSL'));
			$this->redirect($this->url->link('account/login', '', 'SSL'));
		}
		$this->data['tplpath'] = DIR_DIR . 'view/theme/' . $this->config->get('config_template') . '/';
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/setup.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/setup.tpl';
		} else {
			$this->template = 'default/template/account/setup.tpl';
		}
