...
class Common_util_
{
	...
	function __construct($appid, $appsecret, $partnerid, $apikey) {
		$this->_appid = $appid;
	...
	function formatBizQueryParaMap($paraMap, $urlencode)
	{
		...
		foreach ($paraMap as $k => $v)
		{
			...
			// guoxiaoan fixed $buff .= strtolower($k) . "=" . $v . "&";
			$buff .= $k . "=" . $v . "&";
		}
		$reqPar;
		if (strlen($buff) > 0) 
	 ...
	 * 	作用：格式化参数，签名过程需要使用
	 */
	function formatBizQueryParaMapForUrl($paraMap, $urlencode)
	{
		$buff = "";
		ksort($paraMap);
		foreach ($paraMap as $k => $v)
	...
	public function getSign($Obj)
	{
		foreach ($Obj as $k => $v)
		{
			// guoxiaoan fixed $Parameters[strtolower($k)] = $v;
			$Parameters[$k] = $v;
		}
		//签名步骤一：按字典序排序参数
		ksort($Parameters);
	...
	function postXmlSSLCurl($xml,$url)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, DIR_APPLICATION . 'controller/payment/wxpay_class/cacert/apiclient_cert.pem');
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		//curl_setopt($ch,CURLOPT_SSLKEY, WxPayConf::SSLKEY_PATH);
		curl_setopt($ch,CURLOPT_SSLKEY, DIR_APPLICATION . 'controller/payment/wxpay_class/cacert/apiclient_key.pem');
		//post提交方式
 ...
 * 请求型接口的基类
 */
class Wxpay_client_ extends Common_util_ 
{
	...
	function __construct($appid, $appsecret, $partnerid, $apikey)
	{
		parent::__construct($appid, $appsecret, $partnerid, $apikey);
	}
	...
	function createXml()
	{
	   	...
	   	$this->parameters["mch_id"] = $this->_partnerid;//商户号
 ...
 * 统一支付接口类
 */
class UnifiedOrder extends Wxpay_client_
{	
	function __construct($appid, $appsecret, $partnerid, $apikey) 
	{
		//设置接口链接
 ...
 * 订单查询接口
 */
class OrderQuery extends Wxpay_client_
{
	function __construct($appid, $appsecret, $partnerid, $apikey)
	{
		//设置接口链接
 ...
 * 退款申请接口
 */
class Refund extends Wxpay_client_
{
 ...
 * 退款查询接口
 */
class RefundQuery extends Wxpay_client_
{
 ...
 * 对账单接口
 */
class DownloadBill extends Wxpay_client_
{
 ...
 * 短链接转换接口
 */
class ShortUrl extends Wxpay_client_
{
	function __construct($appid, $appsecret, $partnerid, $apikey) 
	{
 ...
 * 冲正接口
 */
class Reverse extends Wxpay_client_
{
 ...
 * 被扫支付接口
 */
class MicropayCall extends Wxpay_client_
{
	function __construct($appid, $appsecret, $partnerid, $apikey) 
	{
		//设置接口链接
 ...
 * 响应型接口基类
 */
class Wxpay_server_ extends Common_util_ 
{
 ...
 * 请求商家获取商品信息接口
 */
class NativeCall extends Wxpay_server_
{
	function __construct($appid, $appsecret, $partnerid, $apikey) 
	{
		parent::__construct($appid, $appsecret, $partnerid, $apikey);
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $this->_appid;//公众账号ID
		   	$this->returnParameters["mch_id"] = $this->_partnerid;//商户号
 ...
 * 静态链接二维码
 */
class NativeLink  extends Common_util_
{
...
class JsApi extends Common_util_
{
	...
	function createOauthUrlForCode($redirectUrl)
	{
		...
		$bizString = $this->formatBizQueryParaMapForUrl($urlObj, false);
