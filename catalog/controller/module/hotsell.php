...
class ControllerModuleHotsell extends Controller {
	protected function index($setting) {
		...
		if($cate_sql!='')
			$sql.=$cate_sql;
		$sql.=" LEFT JOIN " . DB_PREFIX . "product p ON (p.product_id = op.product_id)  WHERE p.status = '1' AND p.date_available <= NOW() GROUP BY op.product_id ORDER BY p.feature,count DESC  LIMIT " . (int)$setting['limit'];
	...
	public function products($setting){
		$this->load->model('catalog/product');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/hotsell_home.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/module/hotsell_home.tpl';
		} else {
			$this->template = 'default/template/module/hotsell_home.tpl';
		}
