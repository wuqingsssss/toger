...
class ControllerProductCategory extends Controller {  
	public function index() { 	
		...
		if (isset($this->request->get['path'])) {
			...
			if($category_id=='1'||$category_id=='56')
		...
		// 增加菜票活动专栏 #TBD
		if(isset($this->session->data['freepromotion'])){
		    $data_period['filter_category_id'] = 1;
		    $product_total = $this->model_catalog_product->getTotalSupplyProducts($data_period);
		    $this->data['categories'][] = array(
		        'name'  => '菜票专栏' . ' (' . $product_total . ')',
		        'href'  => $this->url->link('product/category', 'path=' .'1' . $url)
		    );
		}
		...
		$data = array(
			'filter_start_date' => $data_period['filter_start_date'],
			'filter_end_date' => $data_period['filter_end_date'],
			'filter_supply_period_id'=>$data_period['filter_supply_period_id'],
		...
		$product_total = $this->model_catalog_product->getTotalSupplyProducts($data); 
		
		$results = $this->model_catalog_product->getSupplyProducts($data);
	...
	public function filter(){
  	...
  	public function lists(){
		...
		$data['filter_start_date']=$period['start_date'];
		$data['filter_end_date']  =$period['end_date'];
		$data['filter_supply_period_id']  =$period['id'];
		$this->data['current_period']    =$period;
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/category_lists.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/product/category_lists.tpl';
		} else {
			$this->template = 'default/template/product/category_lists.tpl';
		}
