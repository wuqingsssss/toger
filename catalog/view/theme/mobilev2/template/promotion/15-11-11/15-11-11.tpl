<?php echo $header;?>
 <link href="<?php echo HTTP_CATALOG.$tplpath;?>template/promotion/15-11-11/css/common.css" rel="stylesheet"/>
    <!-- 页面自定义样式 -->
    <link href="<?php echo HTTP_CATALOG.$tplpath;?>template/promotion/15-11-11/css/home.css" rel="stylesheet"/>
	...
	<div id="header">
	    <div class="pull-left">
	        <a class="return" href="index.php?route=common/home"></a>
	    </div>
	...
	<div class="types_one position-r">
		...
		<div class="headline_bj">
			...
			<?php foreach( $group as $key=> $product){ ?>
				<a onclick="addToCart('<?php echo $product['product_id']; ?>');" class="inline-block bg-white">
					<div class="clearfix">
						<div class="img-wrapper round position-r">					
							<img id="pdt_img<?php echo $product['product_id']; ?>" src="<?php echo $product['thumb']; ?>"/>
                ...
                <div class="prices">
                 <?php if (!empty($product['promotion']['promotion_code'])) { ?>                   
                    <span class="fz-16 col-pink fl"><?php echo $product['promotion']['promotion_price']; ?></span>
								<span class="fz-12 col-black line-through fl eleven_price"><?php echo $product['price'];?></span>
                     <?php } else { ?>
