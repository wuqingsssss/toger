...
class ModelSaleOrderRefund extends Model {
    ...
    public function getOrderRefundByTradeNo($ordertradeno,$batch_no) {
    	$sql = "SELECT t.* FROM " . DB_PREFIX . "order_refund t  LEFT JOIN " . DB_PREFIX . "order_payment op ON t.order_payment_id=op.order_payment_id
    			 WHERE op.payment_trade_no='" . $ordertradeno . "' AND t.batch_no='{$batch_no}' ";
    	$query = $this->db->query($sql);
    	$rows = $query->rows;
    	return isset($rows) && count($rows) > 0 ? $rows[0] : null;
    }
    ...
    {
        $sql .= " WHERE o.order_status_id in(13,11,8) AND o.is_delete = '0'";
        
        if (isset($data['filter_order_refund_status']) && !is_null($data['filter_order_refund_status'])) {
    ...
    public function getOrderRefunds($data) {
    	$sql = "SELECT t.*,op.payment_trade_no,op.payment_code as payment_code1,op.value as value1,o.date_added,o.telephone,o.shipping_mobile,o.customer_id,o.date_added,o.date_modified,o.order_type,o.shipping_time,o.pdate,o.partner_code,o.source_from FROM " . DB_PREFIX . "order_refund t
