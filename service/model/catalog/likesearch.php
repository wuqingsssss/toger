<?php
class ModelCatalogLikesearch extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (!$product_data) {
			...
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				...
				$sql .= " ORDER BY p.featured DESC,p.sort_order,p.product_id";	
			}
	...
	public function getFeaturedProductIds($limit) {
		$sql="SELECT p.product_id FROM " . DB_PREFIX . "product p WHERE p.featured='1' AND p.status = '1' AND p.date_available <= NOW()";
	...
	//  like query   add by lance 2014-01-08 begin 
	public function getTotalLikeProducts($data = array()) {
		$sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		
		$sql=$this->getFilterLikeSql($sql,$data);
	...
	private function getFilterLikeSql($sql,$data=array()){
...
//			}
		...
		if((isset($data['filter_model']) && !is_null($data['filter_model']))||(isset($data['filter_subtitle']) && !is_null($data['filter_subtitle']))||(isset($data['filter_description']) && !is_null($data['filter_description'])))
		{
			if (isset($data['filter_model']) && !is_null($data['filter_model'])) {
				$sql .= " AND (p.model LIKE '%" . $data['filter_model'] . "%'";
			}
			
			if (isset($data['filter_subtitle']) && !is_null($data['filter_subtitle'])) {
				$sql .= " OR pd.subtitle LIKE '%" . $data['filter_subtitle'] . "%'";
			}
			
			if (isset($data['filter_description']) && !is_null($data['filter_description'])) {
				$sql .= " OR pd.description LIKE '%" . $data['filter_description'] . "%')";
			}
		}
	...
	public function getLikeProducts($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	// like query   add by lance 2014-01-08 end
