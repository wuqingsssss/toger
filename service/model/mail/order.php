...
class ModelMailOrder extends Model {
	public function send( $order_id ){
		...
		if ($order_status_query->num_rows) {
			$code = $order_status_query->row['code'] ;

			try{
			    $this->$code($order_info,$order_status_query->row['name']);
			}catch(Exception $e){
                $this->log_sys->info('send mail fail: '.$e);
			}
		}
	...
	public function unpay( $order_info,$status='' ){
		// send mail template for order status =payed
		$language = new Language($order_info['language_directory']);
		$language->load($order_info['language_filename']);
		$language->load('mail/order_unpay');

		$order_id=$order_info['order_id'];

		$order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . $order_info['order_id'] . "'");
		$order_total_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $order_info['order_id'] . "' ORDER BY sort_order ASC");
		$order_download_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_download WHERE order_id = '" . $order_info['order_id'] . "'");
		...
		$template->data['text_pdate'] = $language->get('text_new_pdate');
		...
		$template->data['pdate'] = $order_info['pdate'];
		...
		foreach ($order_product_query->rows as $product) {
			...
			$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . $order_id . "' AND order_product_id = '" . (int)$product['order_product_id'] . "'");
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/order_unpay.tpl')) {
			$html = $template->fetch($this->config->get('config_template') . '/template/mail/order_unpay.tpl');
		} else {
			$html = $template->fetch('default/template/mail/order_unpay.tpl');
		}
		...
		// Text Mail
			...
			$text .= $language->get('text_new_order_status') . ' ' . $status . "\n\n";
			...
			foreach ($order_product_query->rows as $result) {
				...
				$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . $order_info['order_id'] . "' AND order_product_id = '" . $result['order_product_id'] . "'");
...
//			$order_total = $result['text'];
			...
			if ($order_info['customer_id']) {
				...
				$text .= $order_info['store_url'] . 'index.php?route=account/invoice&order_id=' . $order_info['order_id'] . "\n\n";
			}
	...
	public function payed( $order_info,$status='' ){
		// send mail template for order status =payed
		$language = new Language($order_info['language_directory']);
		$language->load($order_info['language_filename']);
		$language->load('mail/order_payed');
		...
		$template->data['text_powered'] = $language->get('text_new_powered');
		$template->data['text_pdate'] = $language->get('text_new_pdate');
		$template->data['text_pickupcode'] = $language->get('text_pickupcode');
		...
		$template->data['telephone'] = $order_info['telephone'];
		$template->data['pdate'] = $order_info['pdate'];
		$template->data['pickupcode'] = $order_info['pickupcode'];
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/order_payed.tpl')) {
			$html = $template->fetch($this->config->get('config_template') . '/template/mail/order_payed.tpl');
		} else {
			$html = $template->fetch('default/template/mail/order_payed.tpl');
		}
	...
	function success( $order_info ,$status=''){
		// send mail template for order status =payed
		$language = new Language($order_info['language_directory']);
		$language->load($order_info['language_filename']);
		$language->load('mail/order_success');
		...
		if ($order_info['comment']) {
			...
			$message .= strip_tags(html_entity_decode($order_info['comment'], ENT_QUOTES, 'UTF-8')) . "\n\n";
		}
	...
	public function refund( $order_info ,$status=''){
		// send mail template for order status =payed
		$language = new Language($order_info['language_directory']);
		$language->load($order_info['language_filename']);
		$language->load('mail/order_refund');
	...
	public function cancel( $order_info ,$status=''){
		// send mail template for order status =payed
		$language = new Language($order_info['language_directory']);
		$language->load($order_info['language_filename']);
