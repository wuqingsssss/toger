...
class ALIOSS{
    ...
    public function __construct($access_id = NULL, $access_key = NULL, $hostname = NULL, $security_token = NULL){
        if (!$access_id && !defined('ALI_OSS_ACCESS_ID')){
            throw new OSS_Exception(NOT_SET_OSS_ACCESS_ID);
        }
        if (!$access_key && !defined('ALI_OSS_ACCESS_KEY')){
            throw new OSS_Exception(NOT_SET_OSS_ACCESS_KEY);
        }
        if($access_id && $access_key){
            $this->access_id = $access_id;
            $this->access_key = $access_key;
        }elseif (defined('ALI_OSS_ACCESS_ID') && defined('ALI_OSS_ACCESS_KEY')){
            $this->access_id = ALI_OSS_ACCESS_ID;
            $this->access_key = ALI_OSS_ACCESS_KEY;
        }else{
        ...
        else if (!$hostname and defined('ALI_OSS_ENDPOINT')) {
            $this->hostname = ALI_OSS_ENDPOINT;
        }else{
            $this->hostname = self::DEFAULT_OSS_ENDPOINT;
        }
        
        if (defined('ALI_OSS_OPEN_BUCKET') && defined('ALI_OSS_OPEN_BUCKET')){
        	$this->open_bucket = ALI_OSS_OPEN_BUCKET;
        }
    ...
    public function list_bucket($options = NULL) {
        ...
        return json_decode(json_encode(simplexml_load_string($response->body)),1);
    }
    ...
    public function clear_objects($bucket, $options = NULL){

    	$objects=array();
    	
    	$filelist=$this->list_object($bucket,$options);
    	if(isset($filelist['Contents'][0]))
    	$objects=array_column($filelist['Contents'],'Key');
    	else 
    	$objects[]=$filelist['Contents']['Key'];

    	if($objects){	$options = array(
    	    'quiet' => false,
    	);
    	$this->delete_objects($bucket,$objects,$options);
    	}
    	return $objects;
    }
