...
!function(a){var b=function(){function b(b,d){"dot"==b?(d='<ol class="dots">',a.each(c.li,function(a){d+='<li class="'+(a===c.i?b+" active":b)+'">'+ ++a+"</li>"}),d+="</ol>"):(d='<div class="',d=d+b+'s">'+d+b+' prev">'+c.o.prev+"</div>"+d+b+' next">'+c.o.next+"</div></div>"),c.el.addClass("has-"+b+"s").append(d).find("."+b).click(function(){var b=a(this);b.hasClass("dot")?c.stop().to(b.index()):b.hasClass("prev")?c.prev():c.next()})}var c=this;c.o={speed:500,delay:3e3,init:0,pause:!0,loop:!0,keys:!1,dots:!1,arrows:!1,prev:"&larr;",next:"&rarr;",fluid:!1,starting:!1,complete:!1,items:"ul",item:"li",easing:"swing",autoplay:!0},c.init=function(d,e){c.o=a.extend(c.o,e),e=c.o,c.el=d,c.ul=d.children(c.o.items),c.max=[d.width()||0,d.height()||0],c.li=c.ul.children(c.o.item).each(function(){var b=a(this),d=b.width(),e=b.height();d>c.max[0]&&(c.max[0]=d),e>c.max[1]&&(c.max[1]=e)});var f=c.ul,g=c.li,h=g.length;return c.i=0,d.css({width:c.max[0],height:g.first().height(),overflow:"hidden"}),f.css({position:"relative",left:0,width:100*h+"%"}),e.fluid?g.css({"float":"left",width:100/h+"%"}):g.css({"float":"left",width:c.max[0]+"px"}),e.autoplay&&setTimeout(function(){e.delay&&(c.play(),e.pause&&d.on("mouseover mouseout",function(a){c.stop(),"mouseout"===a.type&&c.play()}))},e.init||0),e.keys&&a(document).keydown(function(a){switch(a.which){case 37:c.prev();break;case 39:c.next();break;case 27:c.stop()}}),e.dots&&b("dot"),e.arrows&&b("arrow"),e.fluid&&a(window).resize(function(){c.r&&clearTimeout(c.r),c.r=setTimeout(function(){var a={height:g.eq(c.i).height()},b=d.width();f.css(a),a.width=Math.min(Math.round(b/d.parent().width()*100),100)+"%",d.css(a),g.css({width:b+"px"})},50)}).resize(),d.on("touchstart",function(a){a.pageX>a.pageY&&a.pageX<-a.pageY||a.pageX<a.pageY&&a.pageX>-a.pageY?a.preventDefault():d.data("left",c.ul.offset().left/d.width()*100)}).on("touchmove",function(a){var b=100*a.pageX/d.width(),e=100*a.deltaX/d.width();c.ul[0].style.left=parseInt(c.ul[0].style.left.replace("%",""))+e+"%",c.ul.data("left",b)}).on("touchend",function(a){var b=c.ul.data("left");if(Math.abs(b)>30){var d=b>0?c.i-1:c.i+1;(0>d||d>=h)&&(d=c.i),c.to(d)}else c.to(c.i)}),c},c.to=function(b,d){c.t&&(c.stop(),c.play());var e=c.o,g=c.el,h=c.ul,i=c.li,j=c.i,k=i.eq(b);if(a.isFunction(e.starting)&&!d&&e.starting(g,i.eq(j)),k.length&&!(0>b)||e.loop!==f){k.length||(b=0),0>b&&(b=i.length-1),k=i.eq(b);var l=d?5:0|e.speed,m=e.easing,n={height:k.height()},o=!1;o||(o=!0,g.find(".dot").eq(b).addClass("active").siblings().removeClass("active"),g.animate(n,l,m)&&h.animate(a.extend({left:"-"+b+"00%"},n),l,m,function(f){c.i=b,o=!1,a.isFunction(e.complete)&&!d&&e.complete(g,k)}))}},c.play=function(){c.t=setInterval(function(){c.to(c.i+1)},0|c.o.delay)},c.stop=function(){return c.t=clearInterval(c.t),c},c.next=function(){return c.stop().to(c.i+1)},c.prev=function(){return c.stop().to(c.i-1)}};a.fn.unslider=function(c){var d=this.length;return this.each(function(e){var f=a(this),g="unslider"+(d>1?"-"+ ++e:""),h=(new b).init(f,c);f.data(g,h).data("key",g)})},b.version="1.0.0"}(Zepto);