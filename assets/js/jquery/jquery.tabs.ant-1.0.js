...
{var obj=this;
	...
	$(window).scroll(function(){
		...
		if(scroH>=navH){
			obj.css({"position":"fixed","top":0,"left":navL,"width":obj.width()});
		}else if(scroH<navH){
...
contentItem.parent().height(contentItem.height());
contentItem.show().css('position','absolute');
	...
	var show=function()
	         {
			  ...
			  if(s.sshowall&&s.index==0)
			       {contentItem.animate({opacity:1}); }
				   else
				   {
					   contentItem.animate({opacity:0});	 
					     if(s.fadeIn>0)
			  {
					    	 contentItem.eq(s.index).animate({opacity:0.3}).animate({opacity:1}); 
	                   
	           }
			   else
			  {
					   contentItem.eq(s.index).animate({opacity:1});
			   }
				   }
	...
	var hide=function(){
	    ...
	    contentItem.eq(s.index).animate({opacity:0});
	};
...
{
	if(!s.width)s.width=subObject.eq(0).outerWidth();
	if(!s.height)s.height=object.eq(0).outerHeight();
	var height=object.eq(0).outerHeight();
var step=s.step*subObject.eq(0).outerHeight();
object.css({width:s.width+"px",overflow:"hidden"});
subObject.css('float','none');
};
if(s.deriction=="left"||s.deriction=="right")
{
	if(!s.width)s.width=object.eq(0).outerWidth();
	if(!s.height)s.height=subObject.eq(0).outerHeight();
...
var showBox=function(){	
	//alert(s.content);	
   $("#windown-box").remove(); //锟斤拷锟斤拷锟斤拷锟?
	var width = s.width>= 950?this.width=950:this.width=s.width;	    //锟斤拷锟斤拷锟斤拷蟠翱诳锟斤拷
	var height = s.height>= 527?this.height=527:this.height=s.height;  //锟斤拷锟斤拷锟斤拷蟠翱诟叨锟?
	if(s.showWindown) {
		var simpleWindown_html = new String;
			simpleWindown_html = "<div id=\"windownbg\" style=\"height:"+$(document).height()+"px;filter:alpha(opacity=0);opacity:0;z-index: 999901\"></div>";
			simpleWindown_html += "<div id=\"windown-box\">";
			simpleWindown_html += "<div id=\"windown-title\"><h2></h2><span id=\"windown-close\">锟截憋拷</span></div>";
	...
	switch(s.type) {
		...
		$.ajax({
			error:function(){
				$("#windown-content").html("<p class='windown-error'>锟斤拷锟斤拷锟斤拷锟捷筹拷锟斤拷...</p>");
			},
	...
	$("#windownbg").animate({opacity:"0.5"},"normal");//锟斤拷锟斤拷透锟斤拷锟斤拷	alert(s.showbg);
	$("#windown-box").show();
	if( height >= 527 ) {
