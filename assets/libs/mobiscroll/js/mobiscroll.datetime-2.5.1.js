        ...
        preset = function (inst) {
            ...
            if (that.is('input')) {
                ...
                var min = that.attr('min'),
                    max = that.attr('max'),
                    mint = that.attr('mint')||"00:00",
                    maxt = that.attr('maxt')||'23:59';
                if (min) {
                    html5def.minDate = ms.parseDate(format, min+' '+mint);
                }
                if (max) {
                    html5def.maxDate = ms.parseDate(format, max+' '+maxt);
                }
                if (mint) {
                    html5def.minT = mint.split(':');
                }
                if (maxt) {
                    html5def.maxT = maxt.split(':');
                }
            }
                ...
                */
                validate: function (dw, i) {
                    var temp = inst.temp, //.slice(0),
                        mins = { y: mind.getFullYear(), m: 0, d: 1, h: minh, i: 0, s: 0, a: 0 },
                        maxs = { y: maxd.getFullYear(), m: 11, d: 31, h: step(hampm ? 11 : maxh, stepH), i: step(59, stepM), s: step(59, stepS), a: 1 },
                        minprop = true,
                        maxprop = true;
