...
class WeixinAddonModel extends WeixinModel{
	function reply($dataArr, $keywordArr = array()) {
		...
		//如果配置model=2 切入800live 客服
		if($config['model'] == 2){
			$this->send_800($dataArr, $config);
		}
	...
	<TransInfo>
		...
		/**
		 * 800发送消息
		 * @param type $data 用户发送的消息数据
		 */
		private function send_800($data, $config){
			$str = 'qwertyuiopasdfghjklzxcvbnm1234567890';
			$echostr = substr(str_shuffle($str), 0,  mt_rand(4, 6));
			//十三位时间戳
			list($t1, $t2) = explode(' ', microtime());
			$timestamp = $t2.ceil( ($t1 * 1000) );
			
			$nonce = mt_rand(108, 88888);
			$token = $config['token'];
...
//			echo $url;exit;
			$xml_data = wp_file_get_contents('php://input');
//			$xml_data = '<xml>
//								<ToUserName><![CDATA[touser]]></ToUserName>
//								<FromUserName><![CDATA[fromuser]]></FromUserName>
//								<CreateTime>1399197672</CreateTime>
//								<MsgType><![CDATA[youaskservice]]></MsgType>
//								<Content><![CDATA[啊,弄死我吧]]></Content>
//						</xml>';
			$flag = $this->curlGet($url, 'post', $xml_data);
			echo $flag;
		}
