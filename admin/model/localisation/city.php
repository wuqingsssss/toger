...
class ModelLocalisationCity extends Model {
	...
	public function addCity($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "city SET status = '" . (int)$data['status'] . "',name = '" . $this->db->escape($data['name']) . "',name_fix = '" . $this->db->escape($data['name_fix']) . "', code = '" . $this->db->escape($data['code']) . "', zone_id = '" . (int)$data['zone_id'] . "', country_id = '" . (int)$data['country_id'] . "'");
			
		$this->cache->delete('city');
	}

	public function editCity($city_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "city SET status = '" . (int)$data['status'] . "',name = '" . $this->db->escape($data['name']) . "',name_fix = '" . $this->db->escape($data['name_fix']) . "', code = '" . $this->db->escape($data['code']) . "', zone_id = '" . (int)$data['zone_id'] . "', country_id = '" . (int)$data['country_id'] . "' WHERE city_id = '" . (int)$city_id . "'");
...
//	}
	...
	public function getCities($data = array()) {
		...
		SELECT `a`.`city_id` AS `city_id`
		     ...
		     , `b`.`name` AS `city_zone`
			 , `b`.`name_fix` AS `city_zone_fix`
		     , `a`.`name` AS `city_name`
			, `a`.`name_fix` AS `city_name_fix`
		     , `a`.`code` AS `city_code`
