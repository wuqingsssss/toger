...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		...
		foreach ($data['information_description'] as $language_id => $value) {
			if($this->config->get('config_language_id')==$language_id)
				$name=$this->makeSlugs($this->db->escape($value['title']));
		...
		if ($data['keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'information_id=" . (int)$information_id . "', keyword = '" . $this->makeSlugs($this->db->escape($data['keyword'])) . "'");
		}else{
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'information_id=" . (int)$information_id . "', keyword = '" . $name. "'");
		}
		
		if(isset($data['group_id'])){
			$this->editInformationGroup($information_id,$data['group_id']);
		}
	...
	private function editInformationGroup($information_id,$group_id){
		$this->db->query("UPDATE " . DB_PREFIX . "information SET group_id = '" . (int)$group_id . "' WHERE information_id=".(int)$information_id);
	}
	...
	public function getInformations($data = array()) {
		if ($data) {
			...
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				...
				$sql .= " ORDER BY i.group_id,i.sort_order,id.title";	
			}
			...
			if (!$information_data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY i.group_id,i.sort_order,id.title");
