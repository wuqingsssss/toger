                ...
                function storeProductsIntoDatabase( &$reader,&$database, &$products )
                {
                	...
                	foreach ($products as $product) {
                		...
                		if(!$this->checkProductExisted($sku)){
                			...
                			//Add product option
                			if($size){
                				$this->insertProductOption($productId,$size,$price,$quantity);
                			}
...
//                			}
                ...
                private function getOptionValueId($option_id,$value){
                	$sql="SELECT option_value_id FROM " . DB_PREFIX . "option_value_description WHERE option_id=".(int)$option_id." AND name='".$this->db->escape($value)."'";
                	...
                	if($query->num_rows){
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '1', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape($value) . "'");
                ...
                private function checkProductAttributeEmpty($product_id){
                	$sql="SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_attribute WHERE product_id=".(int)$product_id;
                ...
                private function insertProductOption($product_id,$size,$price,$quantity){
                	$option_id=16;
...
//                		$product[0] = $productId;
                ...
                function uploadAttributes( &$reader, &$database )
                {
                	...
                	for ($i=0; $i<$k; $i+=$step) { 
                		...
                		if(!$this->checkProductAttributeEmpty($productId)){
                			continue;
                		}
                		...
                		$this->storeAttributesIntoDatabase( $database, $product_attributes );
                	}
