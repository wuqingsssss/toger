  <style>
    ...
    <div>
     <ul>
      <li class='text'>
      <input type="text" id="prefix" name="prefix" value="" placeholder="请输入路径" class="requiredkeyword" />
        </li>
      <li>buckets：
      <?php  foreach ($buckets as $bucket) {?>
      <input type="radio" name="bucket" checked="checked" value="<?php echo $bucket['Name'];?>" />
      <?php echo $bucket['Name'];?>  
     <?php }?>

          显示数量：<input type="text" name="maxkeys" value="10" size='10' />
          
      <a class="btn btn-primary" onclick="checkoss('clear');" ><?php echo $button_clear;?></a>
      <a class="btn btn-primary" onclick="checkoss('update');" ><?php echo $button_update;?></a>
      <a class="btn btn-primary" onclick="checkoss('upload');" ><?php echo $button_upload;?></a>
      <a class="btn btn-primary" onclick="checkoss('check');" ><?php echo $button_check;?></a>
      <a class="btn btn-primary" onclick="checkoss('find');" ><?php echo $button_done;?></a>
...
{   var prefix =$('#prefix').val();
    ...
    $('input[name="bucket"]:checked').each(function(){
    	bucket=$(this).val();
    }); 

if(!bucket )	$("#msg_keyword").html('<?php echo $error_no_loggers;?>');

	var url='index.php?route=tool/oss/'+action;
	$("#ossdata").html('');
