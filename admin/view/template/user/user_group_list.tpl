  ...
  <div class="box">
    <div class="heading">
      ...
      <div class="buttons">
		  <?php if($is_admin){?>
		  <a onclick="location = '<?php echo $insert; ?>'" class="btn btn-primary">
			  <span><?php echo $button_insert; ?></span>
		  </a> 
		  <?php } ?>
		  <a onclick="$('form').submit();" class="btn"><span><?php echo $button_delete; ?></span>
		  </a>
	  </div>
    </div>
    <div class="content">
          ...
          <tbody>
            ...
            <tr>
              <td style="text-align: center;"><?php if ($user_group['selected']) { ?>
                ...
                <?php } ?></td>
              <td class="left"><?php if(!$user_group['show_add']){?>
				  ——
				  <?php }?><?php echo $user_group['name']; ?></td>
              <td class="right">
				  <?php if (!$super && $user_group['show_add']) { ?>
				  高级分组(不可编辑)
				  <?php }else{?>
