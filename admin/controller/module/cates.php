...
class ControllerModuleCates extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('cates', array());
			
			$this->model_design_layout->updateModules('cates',  $this->request->post['cates_module']);
					
			
			$this->session->data['success'] = $this->language->get('text_success');
			
			$this->redirect($this->url->link('module/cates', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if (isset($this->request->post['cates_module'])) {
			...
			//兼容老版本写法，可以自动升级到新版
			$this->data['modules'] =array_merge($this->config->get('cates_module')?$this->config->get('cates_module'):array(),$this->model_design_layout->getLayoutModules(array('code'=>'cates')));
