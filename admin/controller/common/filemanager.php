...
class ControllerCommonFileManager extends Controller {
	...
	public function index() {
		$this->load_language('common/filemanager');
	...
	public function manager() {
		...
		$this->template = 'common/fullfilemanager.tpl';
	...
	public function files() {
		...
		}else if(isset($this->session->data['directory']) && $this->session->data['directory']){
			$directory = DIR_IMAGE . 'data/' . str_replace('../', '', $this->session->data['directory']);
		} else {
	...
	public function delete() {
		...
		if (!isset($json['error'])) {
			if (is_file($path)) {
			    ...
			    $this->oss->delete_object($this->oss->open_bucket,'image/'.str_replace(DIR_IMAGE,'',$path));
				$this->log_sys->info('filemanager::this->alioss->delete_object::'.$directory . '/' . $filename);
			} elseif (is_dir($path)) {
				//文件夹不可删除
				$json['error'] = '失败:文件夹不可删除';
				return $this->response->setOutput(Json::encode($json));
//				$this->recursiveDelete($path);
	...
	public function move() {
		...
		if (!isset($json['error'])) {
			...
			//$this->load->service ( 'ali/alioss' ,'service');
			    $this->oss->upload_file_by_file($this->oss->open_bucket,'image/'.str_replace(DIR_IMAGE,'',$to . '/' . basename($from)),$from);
			$this->log_sys->info('filemanager::this->alioss->upload_file_by_file::'.'image/'.str_replace(DIR_IMAGE,'',$to . '/' . basename($from)));
	...
	public function copy() {
		...
		if (!isset($json['error'])) {
			if (is_file($old_name)) {
				...
				//$this->load->service ( 'ali/alioss' ,'service');
			    $this->oss->upload_file_by_file($this->oss->open_bucket,'image/'.str_replace(DIR_IMAGE,'',$new_name),$old_name);
				$this->log_sys->info('filemanager::this->alioss->upload_file_by_file::'.'image/'.str_replace(DIR_IMAGE,'',$new_name));
			} else {
	...
	public function upload() {
		...
		if (isset($this->request->post['directory'])) {
			if (isset($this->request->files['image']) && $this->request->files['image']['tmp_name']) {
				...
				$directory = rtrim('data/' . str_replace('../', '', $this->request->post['directory']), '/');
		...
		if (!isset($json['error'])) {	
			...
			if (@move_uploaded_file($this->request->files['image']['tmp_name'], DIR_IMAGE .$directory . '/' . $filename)) {	

				//同步更新到阿里oss
				//$this->load->service ( 'ali/alioss' ,'service');
			    $this->oss->upload_file_by_file($this->oss->open_bucket,'image/'.$directory . '/' . $filename,DIR_IMAGE .$directory . '/' . $filename,NULL);
				$this->log_sys->info('filemanager::upload_file_by_file::'.'image/'.$directory . '/' . $filename);
