...
class ControllerUserUser extends Controller {  
	private $error = array();
  	...
  	private function getList() {
		...
		foreach ($results as $result) {
			...
			$g_info = $this->model_user_user_group->getUserGroup($result['user_group_id']);
	...
	private function getForm() {
		...
		if($this->user->isSuperAdmin()){
			$user_groups=$this->model_user_user_group->get_fist_goup();
		}else {
			$user_groups[]=$this->model_user_user_group->getUserGroup($this->user->getGroupId());
		}

		
    	$this->data['user_groups'] = $user_groups;
    	
		$g_info = $this->model_user_user_group->getUserGroup($user_info['user_group_id']);
		
		$pid = $g_info['parent_group_id'];
		if(empty($pid)){
			$this->data['parent_id'] = $user_info['user_group_id'];
		}else{
			$this->data['parent_id']=$pid ;
		/*
			foreach($this->data['user_groups'] as $g){
				if($pid == $g['user_group_id']){
					$this->data['parent_id'] = $g['user_group_id'];
				}
			}*/
		}
		
		if (isset($this->request->post['sub_id'])) {
      		$this->data['parent_id'] = $this->request->post['sub_id'];
    	}  
  	...
  	private function validateForm() {
		...
		if ((strlen(utf8_decode($this->request->post['sub_id'])) < 1) || (strlen(utf8_decode($this->request->post['sub_id'])) > 5)) {
      		$this->request->post['sub_id'] = $this->request->post['user_group_id'];
    	}
	...
	public function get_sub_group(){
		$id = intval($this->request->get['g_id']);
		$this->load->model('user/user_group');
		$list = $this->model_user_user_group->get_fist_goup($id);
		if(!empty($list)){
			$data['code'] = 0;
			$data['list'] = $list;
		}else{
