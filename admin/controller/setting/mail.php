...
class ControllerSettingMail extends Controller {
	...
	public function index() {
		...
		$this->data['token'] = $this->session->data['token'];
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title_mail'),
			'href'      => $this->url->link('setting/mail', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => $this->language->get('text_breadcrumb_separator')
   		);
		...
		} elseif ($this->config->get('config_smtp_ssl')) {
			$this->data['config_smtp_ssl'] = $this->config->get('config_smtp_ssl');
		} else {
			$this->data['config_smtp_ssl'] = '';
		}
