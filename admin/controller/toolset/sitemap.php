<?php  
class ControllerToolSetSitemap extends Controller {
	public function index() {
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('toolset/sitemap', 'token=' . $this->session->data['token'] , 'SSL'),
      		'separator' => $this->language->get('text_breadcrumb_separator')
   		);
		...
		$this->data['action'] = HTTPS_SERVER . 'index.php?route=toolset/sitemap/creat&token=' . $this->session->data['token'];
		
		
		$this->template = 'toolset/sitemap.tpl';
		$this->id = 'content';
