<?php
class ControllerToolSetAutoSEO extends Controller {
    private $error = array();
    public function index () {
       ...
       $this->load->model('sale/auto_seo');
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			$this->redirect($this->url->link('extension/tool', 'token=' . $this->session->data['token'], 'SSL'));
        }
        ...
        if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('toolset/auto_seo', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => $this->language->get('text_breadcrumb_separator')
   		);
        
        $this->data['action'] =  $this->url->link('toolset/auto_seo', 'token=' . $this->session->data['token'], 'SSL');
